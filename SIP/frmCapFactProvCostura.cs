...
namespace SIP
{
    public partial class frmCapFactProvCostura : Form
    {
        ...
        private bool AgregaNuevoPedidoDgv(int NumeroPedido,int PrendasFletes,string Facturas,decimal CostoEnvio)
        {
            ...
            try
            {
                ...
                renglon["PRENDAS_COSTURA"] = PrendasFletes;
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            ...
            {
                DataTable dataTableDocPedidos           = ProveedoresCostura.RegresaDocumentosPorPedido(int.Parse(txtNumeroPedido.Text));
                DataTable dataTableDocPedidosPrevios    = ProveedoresCostura.RegresaDocumentosPorPedidoPrevios(int.Parse(txtNumeroPedido.Text));
                ...
                {
                    if ((int)dataTableDocPedidos.Rows[0]["PRENDAS_COSTURA"] != 0)
                    {
                        bool renglonAgregado = AgregaNuevoPedidoDgv(
                            (int) dataTableDocPedidos.Rows[0]["PEDIDO"],
                            (int)dataTableDocPedidos.Rows[0]["PRENDAS_COSTURA"],
                    ...
                    {
                        lblMsg.Text = "El Pedido no contiene procesos de Costura.";
                    }
                }
                else
                {
        ...
        private void CalculaTotales()
        {
            ...
            foreach (DataRow renglonPedidoDoc in dataTableDocumentosPedidos.Rows)
            {
                try
                {
                    totalPrendas += (int)renglonPedidoDoc["PRENDAS_COSTURA"];
                    costoPrevio += (int)renglonPedidoDoc["PRENDAS_COSTURA"] * (decimal)renglonPedidoDoc["COSTO_PREVIO"];
                }
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            if (CamposCompletos())
            {
                ...
                {
                    bool informacionGuardada = ProveedoresCostura.GuardaInformacionCostura(
                        txtClave.Text,
                        txtFactura.Text,
