/*
01-Abr-2015 : Se cambia tipo de datos INT a FLOAT para que mustre decimales cuando el Requerimiento no se cumple
11-May-2015 : Se aumentó el tamaño de una variable Varchar porque al hacer Convert de un float se desbordaba
*/
	...
	BEGIN
		...
		IF (select count(CVE_ART) from #tblVerificacion)=0
			BEGIN
				...
				into #tblExist
				FROM #principal p left outer join aspel_sae50.dbo.MULT01 m on p.COMPONENTE = LTRIM(RTRIM(m.CVE_ART))
				where m.CVE_ALM = 2 and convert(numeric(12,2),REQUERIMIENTO) > convert(numeric(12,2),m.EXIST)
					...
					BEGIN
						BEGIN TRY
							BEGIN TRAN
										...
										declare @TBLCONTROL01  NUMERIC(10)
										SELECT @TBLCONTROL01 = ULT_CVE FROM aspel_sae50.dbo.TBLCONTROL01 WHERE ID_TABLA = 44 
										...
										SELECT DISTINCT p.COMPONENTE, 2, @TBLCONTROL01 + row_number() OVER(ORDER BY p.REQUERIMIENTO), 53, cast(getdate() as date), 'N', p.REFERENCIA, '', '', p.REQUERIMIENTO, 
										...
										p2.CLAVE, pd.PROCESO, getdate(), 0, 5, pd.COMPONENTE, 2, 
				...
				ELSE
					BEGIN
						...
						set @resultado= 'No hay existencia suficiente de '+ @CVE_ART + ' se requieren ' + convert(varchar(20), @REQUERIMIENTO) + ' y en el almacen 2 sólo hay ' + convert(varchar(20), @EXIST)
					END
			END
		ELSE
			BEGIN
